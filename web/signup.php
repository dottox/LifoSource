...
function web() {
	echo '<p id="page-desc">Bienvenido al inicio!</p>';
...
function process_form() {
	...
	$line = searchUserByUsername($uname, $upass);

	if (!$line) {
		$myfile = fopen("database.txt", "a");
		$rand = rand(0, 999999999);
		$text = "\n$uname:$upass:$rand:100:10:10";
		fwrite($myfile, $text);
		$_SESSION["msg"] = "Su cuenta fue creada correctamente.";
		header('Location: inicio.php');
		exit();
	} else {
